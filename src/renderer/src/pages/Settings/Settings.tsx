...
export const Settings = () => {
  ...
  const loadSettings = async () => {
    try {
      const config = await invoke<SettingsConfig>('settings_get')
  ...
  const handleSelectFolder = async () => {
    try {
      setIsLoading(true)
      const folder = await invoke<string | null>('settings_select_db_folder')
      if (folder) {
        await invoke('settings_save_db_path', { folderPath: folder })
        const newConfig = await invoke<SettingsConfig>('settings_get')
