    ...
    <Document>
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.infoBlock}>
          <Text style={styles.infoItem}>Atendimentos realizados: {atendimentoCount} ({formatBRL(totalAttendances)})</Text>
          <Text style={styles.infoItem}>Cestas distribuídas: {cestasCount} ({formatBRL(cestasCount * basketValue)})</Text>
        </View>
      ...
      {/* PAGE 2: DETALHAMENTO ESTILO PROTOCOLO */}
      <Page size="A4" orientation="landscape" style={[styles.page, { fontSize: 7 }]}>
        {/* HEADER */}
        <View style={styles.header}>
          <Image src={logoPDF} style={styles.logo} />
          <Text style={styles.institution}>{institutionName}</Text>
            ...
            {unityBreakdown.map((group) => (
              ...
              >
                ...
                {group.items.map((item) => {
                  ...
                  const tags = [hasMinist ? 'A' : '', hasCesta ? 'C' : ''].filter(Boolean).join(' ')
                  return (
                    <View
                      key={item.id}
                      wrap={false}
                ...
                >
                  <Text style={{ fontSize: 6, fontWeight: 'bold', color: '#1a3a6b' }}>
                    Total: {formatBRL(group.totalCost)}
                  </Text>
                  <Text style={{ fontSize: 6, color: '#555' }}>
                    {group.atendimentosCount} atend. / {group.cestasCount} cestas
                  </Text>
                </View>
              </View>
            ))}
        ...
        {unityBreakdown.length > 0 && (
          ...
          >
            <Text style={{ fontSize: 7, fontWeight: 'bold' }}>
              TOTAL GERAL —{' '}
              {unityBreakdown.reduce((a, g) => a + g.atendimentosCount, 0)} atendimentos |{' '}
              {unityBreakdown.reduce((a, g) => a + g.cestasCount, 0)} cestas
            </Text>
