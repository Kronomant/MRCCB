...
const generateReunions = ( length: number ): Array<Reunion> => {
  return Array.from({ length }, () => ({
    id: faker.number.int(1000),
    name: faker.helpers.arrayElement(['Reunião de Atendimento', 'Reunião de Atendimento Ministério']),
...
const mockDataService = {
  ...
  getTreatmentRepeated: ( EnchiridionId: number) => {
    ...
    return treatments.filter( treatment => ( treatment.enchiridionId === EnchiridionId && treatment.date.match(regex)) );
  },
